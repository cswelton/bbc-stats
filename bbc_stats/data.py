    ...
    def parse(self, project_root_dir):
        ...
        for player_name, rounds in player_scores.items():
            ...
            if player_name in scoring_averages:
                data[player_name]["scoring_average"] = scoring_averages[player_name]
            else:
                data[player_name]["scoring_average"] = 0.0
            ...
            for round in rounds:
                if len(round.get("hole_data", [])) == 18:
                    round_data = {
                        ...
                        "in": sum([round["hole_data"][str(i)]["score"] for i in range(1, 10)]),
                        "out": sum([round["hole_data"][str(i)]["score"] for i in range(10, 19)])
                    }
